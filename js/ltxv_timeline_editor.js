            ...
            if (timeMatch) {
                ...
                parts.slice(1).forEach(p => {
                    ...
                    if (p.startsWith('first:')) {
                        ...
                        // mid:MM:SS:img.png
                        const [_, mTime, mImg] = p.split(':'); // This is weak if path has :, assume simple filenames
                        // Better: mid:00:02:file.png
                        // Wait, split limit?
                        // parts of p: 'mid', '00:02', 'file.png'
                        const sub = p.split(':');
                        if (sub.length >= 3) {
                            const t = timeToSec(sub[1] + ':' + sub[2]); // Reconstruct MM:SS ?? No.
                            // timeToSec expects MM:SS.
                            // sub[1] could be MM, sub[2] SS? mid:00:02:file
                            // Or mid:00:02:file.png -> ['mid', '00', '02', 'file.png']... messy.
                            // Let's assume standard format generated by this tool:
                            // mid:TIME:FILE
                            // Time in seconds? Or MM:SS? 
                            // Serializer uses MM:SS usually.

                            // Let's look for time pattern in P
